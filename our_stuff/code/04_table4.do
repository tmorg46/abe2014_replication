...
matrix define tab6col1_beta_can = r(table)[1,2..6]' // this puts the coefficients from the yrsusa2 fixed effects into a column vector we can use later
...
matrix define tab6col2_3_beta_can = r(table)[1,1..13]' // this puts the coefficients from the yrsusa2 fixed effects into a column vector we can use later
...
putexcel set "${route}/output/tables", sheet(table6_can, replace) modify // we're replacing the table4 sheet but modifying the document as a whole; this avoids deleting other tables later but still lets us refresh this table on each rerun
...
matrix define tab6col1_beta_wom = r(table)[1,2..6]' // this puts the coefficients from the yrsusa2 fixed effects into a column vector we can use later
...
matrix define tab6col2_3_beta_wom = r(table)[1,1..13]' // this puts the coefficients from the yrsusa2 fixed effects into a column vector we can use later
...
*now it's time to put together the table into an Excel file:
putexcel set "${route}/output/tables", sheet(table6_wom, replace) modify // we're replacing the table4 sheet but modifying the document as a whole; this avoids deleting other tables later but still lets us refresh this table on each rerun
